...
function playTone(kind: 'good' | 'bad' | 'click') {
  try {
    const AudioContextClass = window.AudioContext || (window as unknown as { webkitAudioContext: typeof AudioContext }).webkitAudioContext
    const ctx = new AudioContextClass()
    const osc = ctx.createOscillator()
    const gain = ctx.createGain()
    osc.frequency.value = kind === 'good' ? 660 : kind === 'bad' ? 170 : 380
...
export default function App() {
  ...
  const go = (next: Screen) => { if (!muted) playTone('click'); setScreen(next) }

  function enterGame(e: React.FormEvent) {
    e.preventDefault()
    if (!player.trim()) return
    setProgress(p => ({ ...p, player: player.trim(), section: section.trim() }))
    go('map')
  }
    ...
    <main className="gate">
      ...
      <section className="gate-card">
        ...
        <form onSubmit={enterGame}>
          ...
          <label>Seksyon <span>(opsyonal)</span><input value={section} onChange={e => setSection(e.target.value)} placeholder="Hal. Sampaguita" /></label>
  ...
  return (
    <main className="app-shell">
      ...
      {screen === 'order' && <OrderMission level={current} picks={orderPicks} choose={chooseOrder} onBack={() => go('level')} />}
...
function LevelBrief({ level, viewed, onView, onOrder, onBack }: { level: Level; viewed: boolean; onView: () => void; onOrder: () => void; onBack: () => void }) {
  ...
  return <div className="mission-page">
    ...
    <section className="mission-grid">
      ...
      <article className={!viewed ? 'mission locked-card' : 'mission'}><span>02</span><h3>Ayusin ang Panahon</h3><p>Piliin ang mga konsepto sa tamang pagkakasunod-sunod.</p><button disabled={!viewed} onClick={onOrder}>{viewed ? 'Simulan ang Hamon' : 'Tapusin muna ang Suri-Aral'}</button></article>
...
function OrderMission({ level, picks, choose, onBack }: { level: Level; picks: string[]; choose: (item: string) => void; onBack: () => void }) {
  return <div className="challenge-page">
    <button className="back" onClick={onBack}>← Bumalik</button>
    <p className="eyebrow">HAMON 02 • AYUSIN ANG PANAHON</p><h2>{level.orderPrompt}</h2><p className="hint">Piliin isa-isa ang sagot sa tamang pagkakasunod-sunod. Magre-reset kapag nagkamali.</p>
